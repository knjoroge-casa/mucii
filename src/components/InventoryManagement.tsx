...
const InventoryManagement = ({ inventoryItems, setInventoryItems, onGenerateShoppingList, activeUserRole = 'viewer', activeHomeId }) => {
  ...
  const [itemForm, setItemForm] = useState({
    ...
    lowStockThreshold: '', storageLocation: '', autoAddToShopping: false
  });
    ...
    try {
      if (editingItem) {
        const { data, error } = await supabase
          ...
          .single();
        if (error) throw error;
        setInventoryItems(prev => prev.map(item => item.id === editingItem.id ? mapDbItem(data) : item));
      } else {
  ...
  const handleEdit = (item) => {
    setItemForm({
      ...
      currentStock: item.currentStock.toString(), unit: item.unit,
      lowStockThreshold: item.lowStockThreshold.toString(),
      storageLocation: item.storageLocation || '', autoAddToShopping: item.autoAddToShopping
    });
    setEditingItem(item);
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-purple-900 to-amber-600 bg-clip-text text-transparent">Inventory</h1>
        ...
        <div className="flex space-x-3">
          ...
          {can.canAddInventoryItem && (
            <button onClick={() => { resetForm(); setEditingItem(null); setShowItemForm(true); }}
      ...
      {filteredItems.length > 0 ? (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center flex-wrap gap-3 mb-2">
                        ...
                        <span className="px-3 py-1 bg-purple-100 text-purple-800 text-xs font-medium rounded-full">{item.category}</span>
                        <span className={`px-3 py-1 text-xs font-medium rounded-full border ${stockStatus.bg} ${stockStatus.color}`}>{stockStatus.label}</span>
        ...
        <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-12 border border-white/50 shadow-lg text-center">
          ...
          <p className="text-gray-500 mb-6">
            {searchTerm || selectedCategory !== 'all' || showLowStockOnly ? 'Try adjusting your search or filters' : 'Start tracking your household essentials'}
          </p>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl">
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-6">Are you sure you want to delete <span className="font-semibold">"{deleteConfirmItem.name}"</span>? This cannot be undone.</p>
              <div className="flex justify-center space-x-4">
                ...
                <button onClick={() => handleDeleteConfirm(deleteConfirmItem.id)} className="bg-red-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-red-700 transition-all">Delete</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900">{editingItem ? 'Edit Item' : 'Add Inventory Item'}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" required min="0" value={itemForm.currentStock}
              ...
              <div>
                ...
                <input type="number" required min="0" value={itemForm.lowStockThreshold}
              ...
              <div className="flex justify-end space-x-4 pt-6 border-t">
                <button type="button" onClick={() => { setShowItemForm(false); setEditingItem(null); resetForm(); }}
