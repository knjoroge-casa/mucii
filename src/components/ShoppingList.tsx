...
const ShoppingList = ({ inventoryItems = [], onUpdateInventory, shoppingItems, setShoppingItems, activeUserRole = 'viewer', activeHomeId }) => {
  ...
  const filteredPending = pendingItems.filter(i => selectedCategory === 'all' || i.category === selectedCategory);
  ...
  const handleMarkPurchased = async (item) => {
    setShoppingItems(prev => prev.map(i => i.id === item.id ? { ...i, completed: true } : i));
    try {
              ...
              <div key={item.id} className="p-5 hover:bg-gray-50/50 transition-all">
                <div className="flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-400 mt-0.5">{item.quantity} {item.unit}{item.preferredStore && ` · ${item.preferredStore}`}</p>
                  </div>
                  <div className="flex space-x-1 ml-4">
