...
const TaskManagement = ({ tasks, setTasks, activeUserId, activeUserRole = 'viewer', activeHomeId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingTask) {
        ...
        setTasks(prev => [{
          ...taskForm, id: Date.now(), completed: false,
  ...
  const handleComplete = async (task) => {
    ...
    setTasks(prev => prev.map(t =>
      t.id === task.id ? { ...t, completed: nowCompleted, completedAt: updates.completed_at, completedBy: updates.completed_by } : t
    ));
    try {
  ...
  const handleEdit = (task) => {
    setTaskForm({
      title: task.title, description: task.description, zone: task.zone,
      frequency: task.frequency, weekDays: task.weekDays, priority: task.priority,
      assignee: task.assignee, date: task.date
    });
    setEditingTask(task);
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-purple-900 to-amber-600 bg-clip-text text-transparent">Task Management</h1>
              ...
              {['all', ...uniqueZones].map(zone => (
                <button key={zone} onClick={() => setSelectedZone(zone)}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all border ${
                    selectedZone === zone ? 'bg-purple-100 text-purple-800 border-purple-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 border-gray-200'
                  }`}>
          ...
          <div className="border-t border-gray-200 pt-4">
            ...
            <div className="flex flex-wrap gap-2">
              {['all', ...uniqueAssignees].map(assignee => (
                <button key={assignee} onClick={() => setSelectedAssignee(assignee)}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all border ${
                    selectedAssignee === assignee ? 'bg-purple-100 text-purple-800 border-purple-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 border-gray-200'
                  }`}>
      ...
      <div>
        ...
        {filteredActiveTasks.length > 0 ? (
          ...
          <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-12 border border-white/50 shadow-lg text-center">
            ...
            <p className="text-gray-500">{activeFilter !== 'all' ? 'Try adjusting your filters' : 'Add a task to get started'}</p>
          </div>
        )}
      </div>
                ...
                <div key={task.id} className="p-4 hover:bg-gray-50/50 transition-all">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex space-x-2 ml-2">
                      {can.canCompleteTask && (
                        <button onClick={() => handleComplete(task)} className="p-2 text-gray-400 hover:text-purple-600 hover:bg-purple-50 rounded-lg transition-all" title="Mark as incomplete">
                          <X className="w-4 h-4" />
                        </button>
                      )}
                      {can.canDeleteTask && (
          ...
          <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl">
            <div className="text-center">
              ...
              <div className="flex justify-center space-x-4">
                <button onClick={() => setDeleteConfirmTask(null)} className="px-6 py-3 border border-gray-200 rounded-xl text-gray-700 font-semibold hover:bg-gray-50 transition-all">Cancel</button>
                <button onClick={() => handleDeleteConfirm(deleteConfirmTask.id)} className="bg-red-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-red-700 transition-all">Delete</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900">{editingTask ? 'Edit Task' : 'Add a Task'}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <textarea value={taskForm.description}
                  ...
                  rows={3} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500"
              ...
              <div>
                ...
                <div className="flex space-x-2">
                  <select required value={taskForm.zone}
                ...
                <div className="grid grid-cols-2 gap-3">
                  {frequencies.map(freq => (
                    <label key={freq.value} className="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
                      <input type="checkbox" checked={taskForm.frequency.includes(freq.value)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {assignees.length > 0 ? (
                    ...
                    <input type="text" value={taskForm.assignee}
                      onChange={(e) => setTaskForm(prev => ({ ...prev, assignee: e.target.value }))}
                      className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500"
              ...
              <div className="flex justify-end space-x-4 pt-6 border-t">
                <button type="button" onClick={() => { setShowTaskForm(false); setEditingTask(null); resetForm(); }}
